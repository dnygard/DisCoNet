...
plotRateOfChange <- function(filename, seqmin=0, seqmax=1, seqstep=0.1, method="distance", pthresh=0.05,timestamp=NULL,outputdir=NULL){
...
for(cor in cormethods){
  ...
  m <- matrix[[1]] 
  p <- matrix[[2]]
  firststep <- NULL #ANU CHANGED
  nameslist <- c() #ANU CHANGED
  # create nameslist
  for(i in seq(seqmin, seqmax, seqstep)){
    if(!is.null(firststep)){
      stepname = paste(firststep,i,sep="-")
      firststep = i
      
      #if(nameslist) 
	  nameslist <- c(nameslist, stepname)
  ...
  for(threshold in seq(seqmin, seqmax, seqstep)){
    ...
    prlist <- list()#ANU ADDED
    # get PR vector
    pr <- doPageRank(medges)
    prlist[[length(prlist)+1]] <- pr['vector']
    
	
	deglist <- list()#ANU ADDED
    # get degree vector
    g <- graph_from_data_frame(medges, directed = FALSE, vertices = NULL)  # convert to igraph graph
	...
	#ANU ADDED BELOW
	if(is.null(atthreshdf)){
	  atthreshdf <- cbind(threshold, numedges, meandegree, mediandegree, meanpr, medianpr, thistransitivity)
	  colnames(atthreshdf) <- c("Threshold", "numedges", "meandegree", "mediandegree", "meanpr", "medianpr", "transitivity")
	}else{
	  tmp.dm <- cbind(threshold, numedges, meandegree, mediandegree, meanpr, medianpr, thistransitivity)
	  colnames(tmp.dm) <- c("Threshold", "numedges", "meandegree", "mediandegree", "meanpr", "medianpr", "transitivity")
	  atthreshdf <- rbind(atthreshdf, tmp.dm)
	}
  ...
  #names(atthreshdf) <- c("Threshold", "numedges", "meandegree", "mediandegree", "meanpr", "medianpr", "transitivity")
  atthreshdf <- as.data.frame(atthreshdf)
  ...
  comparedf <- NULL
  for(i in 1:10){
    casename <- paste(as.character(atthreshdf$Threshold[i]), as.character(atthreshdf$Threshold[i+1]),sep="-")#ANU CHANGED
	...
	if(is.null(comparedf)){
	  comparedf <- cbind(casename, enumratio, meandegratio, mediandegratio, meanprratio, medianprratio, transitivityratio)
	  names(comparedf) <- c("Threshold", "numedges", "meandegree", "mediandegree", "meanpr", "medianpr", "transitivity")
	}else{
	  tmp.dm2 <- cbind(casename, enumratio, meandegratio, mediandegratio, meanprratio, medianprratio, transitivityratio)
	  names(tmp.dm2) <- c("Threshold", "numedges", "meandegree", "mediandegree", "meanpr", "medianpr", "transitivity")
	  comparedf <- rbind(comparedf, tmp.dm2)
	}
  ...
  #names(comparedf) <- c("Threshold", "numedges", "meandegree", "mediandegree", "meanpr", "medianpr", "transitivity")
  comparedf <-  as.data.frame(comparedf)
### plot KS figs
  ...
  for(i in 1:length(nameslist)-1){ #ANU CHANGED
  ...
  plotname <- paste(outputdir,timestamp,"_","KSvDegree", cor, ".png", sep="")
  ...
  plotname <- paste(outputdir,timestamp,"_","KSvPR", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggpr)
  dev.off()
}
  ...
  ggdeg <- ggplot(data = comparedf) + 
    ...
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsEdgenumRateofChange", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = comparedf) + 
    ...
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsDegreeRateofChange", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = comparedf) + 
    ...
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsPageRankRateofChange", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = comparedf) + 
    ...
    geom_line(aes(x=Threshold, y=medianpr, colour="Median", group=1)) +
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsTransitivityRateofChange", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = atthreshdf) + 
    ...
    geom_line(aes(x=Threshold, y=numedges, group=1)) +
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsEdgenum", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = atthreshdf) + 
    ...
    geom_line(aes(x=Threshold, y=mediandegree, colour="Median", group=1)) +
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsDegree", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = atthreshdf) + 
    ...
    geom_line(aes(x=Threshold, y=medianpr, colour="Median", group=1)) +
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsPageRank", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
  ...
  ggdeg <- ggplot(data = comparedf) + 
    ...
    geom_line(aes(x=Threshold, y=medianpr, colour="Median", group=1)) +
    theme_classic()
  plotname <- paste(outputdir,timestamp,"_","metabolomicsTransitivity", cor, ".png", sep="")
  png(plotname, width = 1600, height = 900)
  plot(ggdeg)
  dev.off()
